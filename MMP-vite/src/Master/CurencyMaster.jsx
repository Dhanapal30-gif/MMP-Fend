...
import { deleteCurency, fetchCurency, saveCurencyMaster, updateCurrency } from '../ServicesComponent/Services';
...
const CurencyMaster = () => {
    ...
    const handleSelectAll = (e) => {
        if (e.target.checked) {
            setSelectedRows(curencyMster.map((row) => row.id)); // ✅ Ensure unique selection key
    ...
    const handleRowSelect = (rowKey) => {
        ...
        setSelectedRows((prevSelectedRows) => {
            ...
            if (updatedRows.length === 0) {
                setHandleDeleteButton(false); // Disable delete button if no rows are selected
                setHandleSubmitButton(true); // Enable submit button
                setHandleUpdateButton(false);
            } else {
                setHandleDeleteButton(true); // Enable delete button if rows are selected
